...
class electrode_picker:
    ...
    '''
    def __init__(self, subj_dir, hem, mri_mode):
        '''
        ...
        if mri_mode != 'mri':
            ct_data[ct_data<np.percentile(ct_data.ravel(), 97)]=np.nan
        ...
        for i in np.arange(3):
            ...
            if self.mri_mode == 'mri':
                print("Using MRI mode")
                self.ct_im.append(plt.imshow(ctdat, cmap=cm.hot, aspect='auto'))
            else:
                print("Not using MRI mode")
                self.ct_im.append(plt.imshow(ctdat, cmap=cm.hot, aspect='auto',alpha=0.5, vmin=ctdat.min(), vmax=ctdat.max()))
        ...
        if self.mri_mode == 'mri':
            ...
            curr_min = np.nanmin(self.ct_data.ravel())
        self.ct_slider = Slider(ct_slider_ax, 'CT max', curr_min, np.nanmax(ct_data), facecolor=[0.8, 0.1, 0.1])
...
if __name__ == '__main__':
    ...
    hem = sys.argv[2]
    mri_mode = sys.argv[3]
    e = electrode_picker(subj_dir=subj_dir, hem=hem, mri_mode=mri_mode)
