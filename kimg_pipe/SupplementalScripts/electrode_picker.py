...
class electrode_picker:
    ...
    '''
    def __init__(self, subj_dir, hem, rosa=False):
        '''
        ...
        rosa : bool
            Whether or not to initialize with pre-marked
            electrodes from ROSA trajectories (requires
            other preprocessing steps – see workflow)
        ...
        if rosa:
            # Load and plot ROSA elecs from self.subj_dir/elecs/rosa/
            # These .mat files are generated by kimg_pipe.freeCoG.reg_rosa_devices()
            # Code in this block based off "n" section of self.on_key()
            rosa_devices = [f.split('.')[0] for f in os.listdir(os.path.join(self.subj_dir, 'elecs', 'rosa')) if f[-3:] == 'mat']
            print(f"Loading elecs from {len(rosa_devices)} ROSA devices...")
            for d in rosa_devices:
                self.device_name = d
                self.devices.append(self.device_name)
                self.elec_num[self.device_name] = 0
                elecfile = os.path.join(self.subj_dir, 'elecs', 'rosa', self.device_name+'.mat')
                # No need to check if path is file
...
if __name__ == '__main__':
    ...
    subj_dir = sys.argv[1]
    hem = sys.argv[2]
    rosa = True if sys.argv[3] == 'True' else False
    e = electrode_picker(subj_dir = subj_dir, hem = hem, rosa = rosa)
